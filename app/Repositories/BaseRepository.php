...
class BaseRepository
{
    ...
    {
        return $this->query()->where('uid',$uuid)->first();
    }
    ...
    /*Query with limit*/
    public function queryWithLimit(array $where_condition, int $limit = 10, string $col = 'created_at')
    {
        return $this->query()->limit($limit)->orderBy($col)->where($where_condition);
    }
...
//    Create mass assign using DB builder
     ...
     * @param  string  $table
     * @param  array   $resource_id   Example: ['id' => '1']
     * @param  array   $input
    ...
    /**
     * Update mass assign by filtering only keys that exist in the table.
     *
     * @param  string  $table
     * @param  array   $where_input   Example: ['uid' => '12345']
     * @param  array   $input
     * @return int|bool   Number of rows updated OR false
     */
