...
@section('content')
    <div class="card radius-10">
        <div class="card-header d-flex align-items-center justify-content-between bg-transparent">
            ...
            <a href="{{ route('admin_panel.expenses.create') }}" class="btn btn-primary btn-sm radius-30 px-3">
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#expenses_table').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('admin_panel.expenses.get_all_for_dt') }}",
                columns: [
                ...
                fnRowCallback: function (nRow, aData) {
                    ...
                    $(nRow).on('click', function (e) {
                        if (!$(e.target).closest('button, a').length) {
                            window.location.href = url + "/admin_panel/expenses/profile/" + aData['uuid'];
                        }
                    });
                }
            });
